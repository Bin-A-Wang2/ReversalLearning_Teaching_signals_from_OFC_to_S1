...
userOptions = defineUserOptions_RL_cue();
compareRDM=0;
%%
for i= 1:length(conditionLabels)
    ...
    if i==1
        userOptions.rootPath = 'D:\Bochum\DATA\fMRI_RL_GoNoGo\Results_OFC_S1\Results_RSA\Results_cue\RDMs_LERN';
    elseif i==2
        userOptions.rootPath = 'D:\Bochum\DATA\fMRI_RL_GoNoGo\Results_OFC_S1\Results_RSA\Results_cue\RDMs_LERE';
    end
    mkdir(userOptions.rootPath)
    ...
    for m=1:size(sRDMs,1)
        for n=1:size(sRDMs,2)
            sRDMs_mod(m,n).RDM=sRDMs(m,n).RDM(1:2,3:4);
        end
    end
    ...
    for j=1:length(mRDMs)
        mRDMs_mod(j).RDM=mRDMs(j).RDM(1:2,3:4);
    end
    
    
    save([userOptions.rootPath,'\RDMs\RL_GoNoGo_RDMs.mat'],'RDMs','sRDMs','mRDMs','sRDMs_mod','mRDMs_mod');
    
    Models_all = rsa.constructModelRDMs(modelRDMs_RL_S1_cue(), userOptions);
